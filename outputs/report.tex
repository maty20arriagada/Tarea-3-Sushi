...
\subsection{Escenario A --- Shock de precio del toro (+40\%)}

\textbf{Intervención}: subimos el precio del toro 40\% y miramos qué pasa con los shares. En este dataset el precio es señal de calidad (no de costo), así que un alza hace al toro \emph{más} atractivo, no menos. Comparamos dos lentes: MNL con constantes calibradas y $\beta$ de M6 (asume que todos los ítems son igual de sustituibles entre sí) vs.\ Nested Logit M4b con $\lambda_{\text{akami}} = 0{,}323$, $\lambda_{\text{seafood\_other}} = 0{,}342$, $\lambda_{\text{non\_seafood}} = 1$ (permite que los ítems del mismo nido sean sustitutos más cercanos).

\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\textwidth]{scenario_A_nl_mnl.png}
  \caption{Escenario A: shares y $\Delta$ (pp) tras +40\% en el precio del toro. El NL predice que los vecinos de nido del toro (maguro, tekka\_maki) pierden \emph{más} share que bajo MNL: consecuencia de $\lambda_{\text{akami}} = 0{,}323$.}
  \label{fig:scenarioA}
\end{figure}
...
\begin{table}[H]
  \centering
  \small
  \caption{Escenario A --- shock de precio del toro: shares y elasticidades}
  \label{tab:scenarioA}
  \begin{tabular}{lrrrc}
    \toprule
    Ítem & Base \% & MNL \% & NL \% & Elasticidad-precio (MNL) \\
    \midrule
    Toro (atún graso)      & 34,3 & 45,0 & 50,5 & 0,781 (propia) \\
    Maguro (atún)          &  8,1 &  6,8 &  3,0 & $-$0,407 \\
    Anago (anguila)        & 11,0 &  9,2 &  9,1 & $-$0,407 \\
    Uni (erizo)            & 14,9 & 12,5 & 12,3 & $-$0,407 \\
    Ikura (huevas salmón)  & 10,9 &  9,1 &  9,0 & $-$0,407 \\
    Ebi (camarón)          &  9,2 &  7,7 &  7,6 & $-$0,407 \\
    Ika (calamar)          &  4,6 &  3,8 &  3,8 & $-$0,407 \\
    Tamago (huevo)         &  4,1 &  3,5 &  3,4 & $-$0,407 \\
    Tekka-maki (rollo atún)&  2,3 &  1,9 &  0,8 & $-$0,407 \\
    Kappa-maki (pepino)    &  0,7 &  0,6 &  0,6 & $-$0,407 \\
    \bottomrule
  \end{tabular}
  \\[4pt]
  {\footnotesize Todas las elasticidades-precio cruzadas del MNL son idénticas ($-$0,407): firma del IIA. Las del NL difieren por nido. \textbf{Caveat}: $\beta_{\text{precio}} > 0$ $\rightarrow$ el alza de precio \emph{eleva} la utilidad del toro (señal de calidad; ver Sección~\ref{sec:price_quality}).}
\end{table}

\textbf{Hallazgo clave}: subirle el precio al toro lo hace más deseado (señal de calidad): su share pasa de 34\% a 45\% (MNL) o 50\% (NL). Los que pierden son sus vecinos de nido ---maguro cae 5,1 pp bajo NL vs.\ 1,3 pp bajo MNL, y tekka\_maki 1,4 pp vs.\ 0,4 pp--- porque $\lambda_{\text{akami}} = 0{,}323$ los hace sustitutos cercanos: cuando el toro sube, arrastra a los suyos. El MNL no captura este efecto porque asume que todos los ítems son igual de intercambiables. La lección práctica: en un menú, los ítems del mismo nido del atún se canibalizan entre sí mucho más de lo que un modelo simple predice.
