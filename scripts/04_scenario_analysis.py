...
Engine (§6.0):  V_j = C_j + beta·X_j
  - Attribute betas taken from the attribute-only MNL on Set A (the model
    labelled "M1" in the report; internal name "M1_attributes"). Its price
    coefficient is inflated by corr(price, oiliness) = 0.82, so price
    scenarios are read qualitatively (direction and substitution ranking),
    not as exact magnitudes — see report limitations.
  - C_j calibrated to reproduce observed first-choice shares at baseline.
  - NL scenarios use the M2 (=M4b) lambdas (lambda_non_seafood = 1 fixed).
  - This engine is needed because no single model estimates ASCs +
    attribute betas together (§0.3 identification restriction).

The former Set B / M6-based scenarios (C: California roll entry, D: Kinki
regional campaign) were removed together with M6; only the two Set A
scenarios remain.

Scenarios:
  A — Toro price shock: price_norm +40%, compare MNL vs NL (Set A)
  B — Kappa promotion: -30% price + freq_sold boost, by age x eastwest (Set A)
...
import config
from config import (
    SETA_CSV, SET_A_ORDER, SET_A_ENGLISH, NESTS, TABLES_DIR, FIGURES_DIR,
)
...
# NL lambdas from M2 (=M4b in the code, lambda_non_seafood fixed at 1: the
# free estimate collapses to the lambda -> 0 boundary, a degenerate-nest
# artifact that is not a credible substitution parameter for prediction).
_nl = pd.read_csv(TABLES_DIR / "nested_logit_results.csv", comment="#")
_nl = _nl[_nl["model"] == "M4b"].set_index("parameter")
lam_nl = np.array([_nl.loc["lambda_akami", "estimate"],
                   _nl.loc["lambda_seafood_other", "estimate"],
                   _nl.loc["lambda_non_seafood", "estimate"]])
# Attribute betas for the Set A scenarios come from the attribute-only MNL
# on Set A ("M1_attributes"; report label M1 -> MNL-atributos). Its price
# coefficient is inflated by corr(price, oil) = 0.82, so price scenarios are
# read qualitatively (direction, substitution ranking), not as exact
# magnitudes. Set B / M6 was removed from the project.
beta_attr_setA = _params("mnl_attributes_results.csv", "M1_attributes",
                         ["b_price_norm", "b_oiliness", "b_freq_sold"])
print(f"  params loaded: attr betas (Set A MNL) {np.round(beta_attr_setA, 3)}, "
      f"lambdas(NL) {np.round(lam_nl, 3)}")
...
print(f"  Set A: {n_users} users, 10 items")
...
# --- MNL calibration ---
C_mnl_a = calibrate_mnl_constants(shares_obs, a_X, beta_attr_setA)
P_mnl_base = mnl_predict(beta_attr_setA, a_X, C_mnl_a)
assert np.abs(P_mnl_base - shares_obs).max() < 1e-10, "MNL calibration failed"

# --- NL calibration ---
C_nl_a, P_nl_base = calibrate_nl_constants(
    shares_obs, a_X, beta_attr_setA, lam_nl, alt_to_nest, len(nest_names))
...
P_mnl_shock = mnl_predict(beta_attr_setA, X_shock, C_mnl_a)
P_nl_shock = nl_predict(C_nl_a, X_shock, beta_attr_setA, lam_nl, alt_to_nest, len(nest_names))
...
with open(TABLES_DIR / "scenario_A_toro.csv", "a", encoding="utf-8") as f:
    f.write(
        f"\n# WARNING: beta_price = {beta_attr_setA[0]:+.3f} > 0 -> price is a "
        "quality signal in this stated-preference survey (no payment). "
        ...
        "a credible substitution parameter for prediction.\n"
        "# Attribute betas transferred from M6 (Set B) with calibrated "
        "constants C_j on Set A observed shares (Train, 2009).\n"
    )
...
# U_nj = C_j + beta_attr_setA·X_nj + beta_int·[price*west, oil*age, oil*west]
# (attribute betas transferred from M6, Set B; maki/nonseafood absorbed by C_j)
C_mnl_b = calibrate_mnl_constants(shares_obs, a_X, beta_attr_setA)

def compute_util(X_cf_attr, price_mat, oil_age_mat, oil_west_mat):
    """Compute V_nj for all users. Uses calibrated C_j + betas."""
    V_attr = (X_cf_attr @ beta_attr_setA).reshape(1, -1)   # (1, 10)
...
with open(TABLES_DIR / "scenario_B_kappa.csv", "a", encoding="utf-8") as f:
    f.write(
        ...
        f"# beta_price = {beta_attr_setA[0]:+.3f} > 0 -> price reduction reduces "
        "utility (quality signal). The share gain comes from the freq_sold "
        f"increase (beta_freq = {beta_attr_setA[2]:+.3f}), which dominates the "
        "price reduction's negative utility effect.\n"
        f"# freq_sold: base {a_freq[kappa_idx]:.2f} -> cf {p75_freq:.2f}\n"
        "# Attribute betas from the Set A attribute MNL (M1_attributes) with "
        "calibrated constants C_j on observed shares (Train, 2009).\n"
    )
